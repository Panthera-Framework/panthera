<?php if(!class_exists('Rain\Tpl')){exit;}?><?php echo localizeDomain('files'); ?>

<?php if( $action == 'display_list' ){ ?>
...
<?php $counter1=-1;  if( isset($files) && ( is_array($files) || $files instanceof Traversable ) && sizeof($files) ) foreach( $files as $key1 => $value1 ){ $counter1++; ?>
            ...
            <div class="uploadBox" id="box_<?php echo $key1; ?>" rel="<?php echo $key1; ?>">
              <div class="boxInner">
                <div class="boxImg"><img src="<?php echo $value1["icon"]; ?>" id="box_img_<?php echo $key1; ?>"></div>
                <div class="titleBox" id="box_title_<?php echo $key1; ?>"><?php echo $value1["name"]; ?></div>
                <input type="hidden" id="box_delete_<?php echo $key1; ?>" value="<?php if( $value1["ableToDelete"] == True ){ ?>1<?php }else{ ?>0<?php } ?>">
                <input type="hidden" id="box_description_<?php echo $key1; ?>" value="<?php echo $value1["description"]; ?>">
                <input type="hidden" id="box_id_<?php echo $key1; ?>" value="<?php echo $value1["id"]; ?>">
                <input type="hidden" id="box_author_<?php echo $key1; ?>" value="<?php echo $value1["author"]; ?>">
                <input type="hidden" id="box_mime_<?php echo $key1; ?>" value="<?php echo $value1["mime"]; ?>">
                <input type="hidden" id="box_type_<?php echo $key1; ?>" value="<?php echo $value1["type"]; ?>">
                <input type="hidden" id="box_link_<?php echo $key1; ?>" value="<?php echo $value1["link"]; ?>">
                <input type="hidden" id="box_directory_<?php echo $key1; ?>" value="<?php echo $value1["directory"]; ?>">
              </div>
            </div>
        <?php } ?>
...
<style>
    ...
    <h2 class="popupHeading"><?php echo localize('Files upload'); ?> - <?php echo localize('Add files from your computer'); ?></h2>
    ...
    <div id="upload_list_window" ondragover="return false;">
      ...
      <div class="uploadToolbox" id="file_informations_window" style="display: none;">
        <div class="uploadToolBoxInner">
            ...
            <b><?php echo localize('Author'); ?>:</b> <i id="file_author">admin</i><br>
            <b><?php echo localize('Type'); ?>:</b> <i id="file_type">document</i><br>
            <b><?php echo localize('Mime-type'); ?>:</b> <i id="file_mime">document/odf</i><br>
            <b><?php echo localize('Directory'); ?>:</b> <i id="file_directory">default</i><br>
            <b><?php echo localize('Description'); ?>:</b> <i id="file_description">This is just a test. File was added from phpMyAdmin but displayed here.</i><br><br>
            <b><?php echo localize('Link'); ?>:</b> <input type="text" id="file_link" value=""><br>
            <input type="hidden" id="file_id"><input type="hidden" id="file_k">
            <b><?php echo localize('Options'); ?>:</b> <input type="button" value="<?php echo localize('Select'); ?>" onclick="selectFile();"> <input type="button" value="<?php echo localize('Remove'); ?>" id="file_delete" disabled>
        </div>
      </div>
      ...
      <div class="optionsBox">&lsaquo;
                        <?php $counter1=-1;  if( isset($pager) && ( is_array($pager) || $pager instanceof Traversable ) && sizeof($pager) ) foreach( $pager as $key1 => $value1 ){ $counter1++; ?>

                            <?php if( $value1 == true ){ ?>

                            <a href="#" onclick="getUploadsPage('page=<?php echo $key1; ?>'); return false;"><b><?php echo $key1+1; ?></b></a>
                            <?php }else{ ?>

                            <a href="#" onclick="getUploadsPage('page=<?php echo $key1; ?>'); return false;"><?php echo $key1+1; ?></a>
                            <?php } ?>

                        <?php } ?>&rsaquo; <?php if( $upload_files == True ){ ?><input type='button' value='<?php echo localize('Add new file'); ?>' style='float: right;' id='_upl_add_file'><?php } ?></div>
  </div>
  ...
  <div id="upload_box_window" style="display: none;">
        ...
        <form action="?display=upload&action=handle_file&popup=true" method="POST" enctype="multipart/form-data" id="upload_form">
            <table class="gridTable">
                <thead>
                    <tr><th colspan="5"><?php echo localize('Upload a new file'); ?></th></tr>
                </thead>
                <tbody>
                    <tr>
                        <td><?php echo localize('Select a file'); ?></td><td><input type="file" name="input_file"> <input type="hidden" name="MAX_FILE_SIZE" value="<?php echo $max_file_size; ?>" /></td>
                    </tr>

                    <tr>
                        <td><?php echo localize('Description'); ?></td><td><input type="text" name="input_description" style="width: 95%;"></td>
                    </tr>
                </tbody>

                <tfoot>
                    <tr>
                        <td colspan="5" class="rounded-foot-left"><em>Panthera - upload <input type="submit" value="<?php echo localize('Send'); ?>" style="float: right;"> <input type="button" id="_upl_back_btn" value="<?php echo localize('Back'); ?>" style="float: right;"></em></td>
                    </tr>
                </tfoot>
            </table>
        </form>
