...
/**
 * Suspend and restore class
 * 
 * Allows suspending and restoring objects
 * 
 * @package Panthera\core\system\kernel
 * @author Damian Kęska
 */
...
abstract class suspendClass
{
    ...
    protected $__lastRestoreID = null;
    
    /**
     * Generate a suspend data
     * 
     * @return string
     */
    
    public function __suspend()
    {
        $panthera = panthera::getInstance();
        ...
        $suspendData = base64_encode('#@pantheraSuspend::' .serialize($args));
        $panthera -> logging -> output('Suspending instance id=' .hash('md4', $suspendData), get_class($this));
        
        return $suspendData;
    }
    
    /**
     * Restore class data from argument
     * 
     * @param string $arg Suspend data (generated by $this->__suspend())
     * @return bool
     */
    
    public function __restore($arg)
    {
        $restoreID = hash('md4', $arg);
        $arg = base64_decode($arg);
        
        if ($arg and substr($arg, 0, 19) == '#@pantheraSuspend::')
            $arg = unserialize(substr($arg, 19, strlen($arg)));
        else
            throw new suspendException('Cannot restore object, suspend header not found, invalid argument', 1);
        
        foreach ($arg as $key => $value)
            $this -> $key = $value;
        
        $panthera -> logging -> output('Restored instance id=' .$restoreID, get_class($this));
        $this -> __lastRestoreID = $restoreID;
