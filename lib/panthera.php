...
class pantheraConfig
{
    ...
    public function __construct($panthera)
    {
        include(SITE_DIR. '/content/config.php'); // TODO: Implement an error handler when there is no $config defined in config.php
        $this->config = $config;
        $this->panthera = $panthera;
        #$this->_loadOverlay();
    ...
    public function __construct() {
        ...
        if (!defined('SITE_DIR'))
        {
            if (MODE == 'CLI')
                ...
                $pathInfo = pathinfo(realpath($_SERVER['SCRIPT_FILENAME']));
            ...
            if (!is_file($path. '/content/config.php'))
            {
                $deep = 0;
                while (True)
                {
        ...
        if (!is_file(SITE_DIR. '/content/config.php'))
            throw new Exception('Cannot find /content/config.php, looking in SITE_DIR=' .SITE_DIR);
...
//            $this->qSerialize = 'binary';
        ...
        $this->config = new pantheraConfig($this);    
...
function pa_exit($string='', $ajaxExit=False)
{
    ...
    if ($ajaxExit == False and MODE == "cgi")
    {
