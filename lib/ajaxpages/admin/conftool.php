...
if ( $_GET['action'] == 'change' ) {
    ...
    $modified = $panthera->get_filters( 'conftool_change', array(
         $key,
        $value 
    ) );
    ...
    if ( !$panthera->config->setKey( $modified[0], $modified[1], $type, $section ) ) {
        ajax_exit( array(
             'status' => 'failed',
            'message' => localize( 'Invalid value for this data type' ) 
        ) );
        pa_exit();
    }
    ...
    ajax_exit( array(
         'status' => 'failed',
        'message' => localize( 'The key propably does not exist', 'conftool' ) 
    ) );
    ...
    ajax_exit( array(
         'status' => 'failed',
        'message' => 'Check your type of value!' 
    ) );
}
...
$sBar->addSetting( 'order', localize( 'Order by', 'search' ), 'select', array(
     'key' => array(
         'title' => localize( 'Key' ),
        'selected' => ( $_GET['order'] == 'key' ) 
    ),
    'section' => array(
         'title' => localize( 'Section', 'conftool' ),
        'selected' => ( $_GET['order'] == 'section' ) 
    ) 
) );
