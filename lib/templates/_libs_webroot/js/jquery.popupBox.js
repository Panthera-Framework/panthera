...
(function($) {
	...
	$.popupBox.prototype = {
		...
		_hook_options: function() {
			for (var pluginName in $.popupBox)
				if (this.is_active(pluginName) && $.popupBox[pluginName].options)
					$.popupBox[pluginName].options.apply(this, arguments);
		},

		// after dom setup
		_hook_domready: function() {
			for (var pluginName in $.popupBox)
				if (this.is_active(pluginName) && $.popupBox[pluginName].domready)
					$.popupBox[pluginName].domready.apply(this, arguments);
		},
		...
		is_active: function(pluginName){
			return !$.popupBox[pluginName] || $.inArray(pluginName, this.opts.pluginsOn) > -1 ||
				( $.inArray(pluginName, this.opts.pluginsOff) < 0 &&
				  ( typeof $.popupBox[pluginName].active == 'undefined' || $.popupBox[pluginName].active )
				);
		}
