...
panthera.googleMap.prototype.getLocation = function (location)
{
    ...
    geocoder.geocode( {'address': location}, function(results, status) {
           if (status == google.maps.GeocoderStatus.OK) {
                  ...
                  {
                      if (results[0].geometry.location.B && results[0].geometry.location.k)
                          _map.setCenter(new google.maps.LatLng(results[0].geometry.location.k, results[0].geometry.location.B));
...
panthera.mSelection = function (selector) {
    mSelectionName = $($(selector)[0]).attr('data-mselection-name');
    arr = sessionStorage.getObject('pantheraMSelection_'+mSelectionName);
    ...
    if (typeof arr == 'object' && arr)
    {
        for (element in arr)
        {
            $('#mSelection_'+mSelectionName+'_'+element).attr('checked', arr[element]);
        }
    }
    ...
    $(selector).change(function (event) {
        ...
        arr = sessionStorage.getObject('pantheraMSelection_'+element.attr('data-mselection-name'));

        if (typeof arr != "object" || !arr)
            arr = {};

        arr[element.attr('data-mselection-id')] = element.attr('checked');
        sessionStorage.setObject('pantheraMSelection_'+element.attr('data-mselection-name'), arr);
        panthera.logging.output('Saving to storage: pantheraMSelection_'+element.attr('data-mselection-name'));
    });
}
