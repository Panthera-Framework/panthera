...
/**
  ...
  *
  * @param json data {url, data, success, messageBox, dataType, mce, before, spinner, async}
  * @return mixed 
  * @author Damian Kęska
  */
...
panthera.jsonPOST = function (input) {
    ...
    if (input.data.substring(0, 1) == '#')
    {
        ...
        {
            formID = input.data;
            input.data = $(input.data).serialize();
        } else
    ...
    if (input.url == undefined || input.url == "")
    {
        ...
        if (formID != "")
            ...
            console.log("panthera.jsonPOST::Emty url argument");
            return false;
        }
    }
    ...
    $.ajax({
      ...
      url: input.url,
      async: input.async,
      /*beforeSend: function(xhr){xhr.setRequestHeader('Panthera-Ajax', 'True');},*/
      data: input.data,
