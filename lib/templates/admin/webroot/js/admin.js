...
panthera.include = function (src, callback, id) {
    if (panthera.debug == true)
...
panthera.googleMap = function (init) {
    // include Google maps API library if not included
    if (typeof google === 'undefined')
    {
        panthera.include('https://maps.googleapis.com/maps/api/js?sensor=false&callback='+init);
        return false;
    }
...
panthera.googleMap.prototype.createMap = function (id, mapOptions) {
    ...
    self.map = new google.maps.Map(document.getElementById(id),this.mapOptions);
}
