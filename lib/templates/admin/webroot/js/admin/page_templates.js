...
function webrootMerge()
{
    ...
    panthera.jsonPOST({ url: '?display=templates&cat=admin&action=webrootMerge', spinner: loader, async: true, success: function (response) {
...
function selectDefaultTemplate(template, caller)
{
    ...
    panthera.jsonPOST({ url: '?display=templates&action=setTemplate&cat=admin&template='+template, spinner: loader, async: true, success: function (response) {
            if (response.status == "success")
                selectTemplate();
            else {
                if (caller != undefined)
...
$(document).ready(function () {
    ...
    $('#template_caching').change(function () {
        ...
        panthera.jsonPOST({ url: '?display=templates&action=exec&cat=admin&name=template_caching&value='+value, spinner: configLoader, async: true, checkbox: '#template_caching'});
    });
    ...
    $('#template_debugging').change(function () {
        panthera.jsonPOST({ url: '?display=templates&action=exec&cat=admin&name=template_debugging&value='+$(this).is(':checked'), spinner: configLoader, async: true, checkbox: '#template_debugging'});
    });
...
function templateTool(toolName, value)
{
    ...
    panthera.jsonPOST({ url: '?display=templates&cat=admin&action=exec&name='+toolName+'&value='+value, spinner: toolsLoader, messageBox: 'messageBox', async: true});
}
