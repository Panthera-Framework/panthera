...
panthera.popup.id = null;
panthera.popup.stack = new Array();

panthera.popup.create = function (link, slot) {
    panthera.popup.stack = cleanArray(panthera.popup.stack, undefined);
    console.log(panthera.popup.stack);
        
    if (!slot || slot == '__default__')
    {
        if (panthera.popup.stack.length)
            slot = panthera.popup.stack[(panthera.popup.stack.length-1)];
        else
            slot = '__default__';
    }
	...
	panthera.popup.id = slot;
	
	if (panthera.popup.stack.indexOf(slot) === -1)
	{
		panthera.logging.output('Added '+slot+' to popup stack');
	    panthera.popup.stack.push(slot);
	    panthera.logging.output(panthera.popup.stack);
	}
...
panthera.popup.close = function (slot) {
    ...
    for (stackItem in panthera.popup.stack)
    {
        if (panthera.popup.stack[stackItem] == slot)
        {
            delete panthera.popup.stack[stackItem];
            panthera.popup.stack = cleanArray(panthera.popup.stack, undefined);
            break;
        }
    }
