    <nav class="tab-fixed">
      <ul class="tab-inner">
        <li><a href="?display=database&cat=admin">{function="localize('Database management', 'database')"}</a></li>
                ...
                <ul class="list inset">
                   ...
                   {loop="$dumps"}
                    <li class="list-item-two-lines">
                     <a href="{$AJAX_URL}?display=sqldump&cat=admin&get={$value.name}&_bypass_x_requested_with">
                        <h3>{$value.name}</h3>
    ...
    function makeDump()
    {
        panthera.jsonPOST({ url: '?display=sqldump&cat=admin', data: 'dump=True', success: function (response) {
                if (response.status == "success")
                    window.location = "?display=sqldump&cat=admin";
            }
        });
        return false;
    }
