            ...
            <li id="gallery" class="tab-item active">
                <ul class="list inset">
                   ...
                   {loop="$functions"}
                     ...
                     {if="$value.type == 'method'"}
                          <li class="list-item-two-lines">
                            <a href="#" onclick="window.location = '?display=browsefile&cat=admin&path={$value.filename}&start={$value.startline}&end={$value.endline}&back_btn={"?display=debhook&cat=admin"|base64_encode}'">
                 ...
                 {else}
                   ...
                   {loop="$hooks"}
                    <li class="list-item-two-lines">
                     <a href="#" onclick="window.location = '{$AJAX_URL}?display=browsefile&cat=admin&path={$value.filename}&start={$value.startline}&end={$value.endline}&back_btn={$where|base64_encode}'">
